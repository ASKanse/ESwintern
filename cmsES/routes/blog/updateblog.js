...
router.post('/:lang',func.auth,function(req, res, next) 
{
  ...
  else
  {
    ...
    con.query("update ? SET ? where blog_id=? and status=1 ;",[langtb,blog,req.body.id],function(err,blogresult,fields)
    {
      if(err)
      {
        console.log(err);
