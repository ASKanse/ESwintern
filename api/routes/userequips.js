...
router.get('/:userid/:equipId',(req,res,next) => {
  ...
  Equip.find({UserID: usrnm, _id: id})
  .select('_id Catagory SubCatagory EquipmentName Manufacturer EqorTrk Model Value PreminumRate Hourly Daily Weekly Monthly Capacity Weight Param1 Param2 EqImg EqLogo')
  .exec()
  .then(info => {
    if(info){
...
router.post('/:userid/create', upload.array('photos', 2),(req,res,next) => {
  const usrnm = req.params.userid;
...
router.patch('/:userid/:equipId', (req, res, next) => {
  ...
  Equip.updateOne({ _id: id , UserID: usrnm}, { $set: updateOps })
    .exec()
    .then(result => {
...
router.delete('/:userid/:equipId', (req,res,next) => {
  const usrnm = req.params.userid;
  const id = req.params.equipId;
  Equip.deleteOne({ _id: id , UserID: usrnm})
    .exec()
    .then(result => {
      res.status(200).json(result);
    })
